...
function assistantTip(){const due=dueItems(),overdue=due.filter(x=>x.days<0),income=monthlyIncomeTotal(),debtLoad=monthlyDebtLoad(),paid=currentMonthPayments().reduce((sum,p)=>sum+p.amount,0),exp=currentMonthExpenses().reduce((sum,x)=>sum+x.amount,0),remaining=Math.max(0,debtLoad-paid),projection=income-debtLoad-exp;if(overdue.length)return {title:'Gecikmiş ödemeleri öne alın',body:`${overdue.length} gecikmiş ödeme var. Bilinen gecikmiş tutar ${money(overdue.reduce((s,x)=>s+x.amount,0))}. Bu ay ${money(paid)} ödeme kaydedildi; planlanan aylık ödemeden ${money(remaining)} kaldı.`};const in7=due.filter(x=>x.days>=0&&x.days<=7);if(in7.length)return {title:'Önümüzdeki 7 gün',body:`Yaklaşan bilinen ödeme ${money(in7.reduce((s,x)=>s+x.amount,0))}. Bu ay şimdiye kadar ${money(paid)} ödendi. Ay sonu tahmini ${money(projection)}.`};if(debtLoad>income&&income>0)return {title:'Aylık ödeme yükü geliri aşıyor',body:`Planlanan aylık borç ödemeleri ${money(debtLoad)}, beklenen gelir ${money(income)}. Bu ay kalan ödeme ${money(remaining)}.`};if(income>0)return {title:'Bu ayın görünümü',body:`Beklenen gelir ${money(income)}. Bu ay ${money(paid)} ödendi, planlanan ödemeden ${money(remaining)} kaldı. Kayıtlı harcamalar sonrası ay sonu tahmini ${money(projection)}.`};return {title:'Gelir planını kontrol edin',body:'Gelirler ve borç ödeme tarihleri güncel olduğunda asistan aylık ödeme baskısını otomatik hesaplar.'}}
...
async function maybeSeedPersonalDebts(){if(!session||appConfig.seededPersonalDebtsV13)return;const existingNames=new Set(state.debts.map(normalizeDebt).map(x=>x.name.toLocaleLowerCase('tr-TR')));for(const seed of PERSONAL_DEBT_SEED){if(existingNames.has(seed.name.toLocaleLowerCase('tr-TR')))continue;await cloudUpsertDebt(normalizeDebt(seed))}appConfig.seededPersonalDebtsV13=true;appConfig.schemaVersion=14;await cloudUpsertAppConfig();await pullCloud();toast('Hazır borç ve gelir planı yüklendi.')}
...
if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=140',{updateViaCache:'none'}).then(reg=>reg.update()).catch(console.error);
