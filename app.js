...
function incomeCard(x){const lesson=x.type==='Özel Ders'&&x.student?` · ${x.student}`:'';return `<article class="list-card clickable" data-income="${esc(x.id)}"><div class="main"><strong>${esc(x.type)}${esc(lesson)}</strong><small>${esc(x.owner)} · ${parseDate(x.date).toLocaleDateString('tr-TR')}${x.addedBy?` · ${esc(x.addedBy)}`:''}</small></div><div class="amount">${money(x.amount)}</div></article>`}
...
async function cloudUpsertIncome(i){return checkErr(await sb.from('gelirler').upsert({id:i.id,user_id:session.user.id,gelir_sahibi:i.owner,gelir_turu:i.type,ogrenci_adi:i.type==='Özel Ders'?(i.student||null):null,gelir_tarihi:i.date,tutar:i.amount,ekleyen:i.addedBy||deviceName(),olusturma_zamani:i.createdAt,guncelleme_zamani:i.updatedAt},{onConflict:'id'}),'Gelir')}
...
$('#incomeForm').addEventListener('submit',async e=>{e.preventDefault();const fd=Object.fromEntries(new FormData(e.target));if(fd.type==='Özel Ders'&&!fd.student)return toast('Özel ders için öğrenci seçin. Öğrencileri Ayarlar’dan ekleyebilirsiniz.');const now=new Date().toISOString(),id=fd.id||uid(),old=state.incomes.map(normalizeIncome).find(x=>x.id===id),obj=normalizeIncome({id,owner:fd.owner,type:fd.type,student:fd.type==='Özel Ders'?fd.student:'',date:fd.date,amount:+fd.amount||0,addedBy:old?.addedBy||deviceName(),createdAt:old?.createdAt||now,updatedAt:now});const i=state.incomes.findIndex(x=>normalizeIncome(x).id===id);if(i>=0)state.incomes[i]=obj;else state.incomes.push(obj);$('#incomeDialog').close();saveState(false);if(session){try{await cloudUpsertIncome(obj);await pullCloud();toast('Gelir kaydedildi.')}catch(er){alert(`Gelir buluta kaydedilemedi: ${er.message}`)}}else{renderAll();toast('Gelir bu telefonda kaydedildi.')}});
...
if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=150',{updateViaCache:'none'}).then(reg=>reg.update()).catch(console.error);
